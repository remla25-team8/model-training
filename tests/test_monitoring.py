...
def test_data_drift_detection(get_splits):
    ...
    def check_drift(train_data, test_data, data_type="feature"):
        ...
        for metric in ['mean', 'std']:
            relative_diff = abs(train_stats[metric] - test_stats[metric]) / (train_stats[metric] if train_stats[metric] != 0 else 1)
            assert relative_diff < 0.5, f"Significant {data_type} drift detected in {metric}: train={train_stats[metric]:.3f}, test={test_stats[metric]:.3f}"
