                ...
                with open(test_file, 'r', encoding='utf-8') as f:
                    ...
                    for category, category_info in TEST_CATEGORIES.items():
                        for test_name in category_info["tests"]:
                            if f"def {test_name}" in content or f"def {test_name.replace('test_', '')}" in content:
...
def calculate_ml_test_score(verbose: bool = False) -> Dict:
    ...
    for category_name, category_info in TEST_CATEGORIES.items():
        ...
        achieved = int((implemented_count / total_tests) * max_score) if total_tests > 0 else 0
        ...
        if verbose:
            ...
            for test in implemented_tests[category_name]:
                print(f"   ✅ {test}")
            missing_tests = set(category_info["tests"]) - set(implemented_tests[category_name])
...
def main():
    ...
    parser.add_argument("--fail-below", type=float, default=0, help="Exit with error if below threshold")
    ...
    try:
        ...
        if not args.verbose:
            print(f"ML Test Score: {results['percentage']}% (Grade: {results['grade']})")
