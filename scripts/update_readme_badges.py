...
def create_badge_markdown(label: str, message: str, color: str, link: Optional[str] = None) -> str:
    ...
    try:
        ...
        if 'pylint_score' in kwargs and 'pylint_color' in kwargs:
            pylint_badge = create_badge_markdown(
                "PyLint", 
                f"{kwargs['pylint_score']:.1f}/10", 
                kwargs['pylint_color']
            )
            content = update_or_add_badge(content, "PyLint", pylint_badge)
            modified = True
            print(f"✅ Updated PyLint badge: {kwargs['pylint_score']:.1f}/10 ({kwargs['pylint_color']})")
        ...
        if 'coverage_percent' in kwargs and 'coverage_color' in kwargs:
            ...
            modified = True
            print(f"✅ Updated Coverage badge: {kwargs['coverage_percent']:.0f}% ({kwargs['coverage_color']})")
        ...
        if 'ml_test_score' in kwargs and 'ml_test_color' in kwargs:
            ...
            modified = True
            print(f"✅ Updated ML Test Score badge: {kwargs['ml_test_score']:.0f}% ({kwargs['ml_test_color']})")
...
Examples:
  ...
  python update_readme_badges.py \\
    ...
    parser.add_argument(
        "--coverage-percent",
        type=float,
        help="Test coverage percentage (0-100)"
    )
    ...
    parser.add_argument(
        "--ml-test-score",
        type=float, 
        help="ML Test Score percentage (0-100)"
    )
    ...
    if args.coverage_percent is not None and args.coverage_color is None:
        print("❌ Error: --coverage-color is required when --coverage-percent is provided")
        sys.exit(1)
    ...
    try:
        ...
        kwargs = {k: v for k, v in vars(args).items() if v is not None and k != 'readme_path'}
